...
public class PlayerAttackTrigger : MonoBehaviour
{
    ...
    private void Awake()
    {
        // 显式判空赋值：UnityEngine.Object 的 == null 能识别 fake null（序列化引用已失效但引用非空），
        // 而 ??= 走 C# 引用 null 检查，遇到 fake null 会短路跳过 GetComponent，导致依赖解析失败
        if (attackCollider == null) attackCollider = GetComponent<Collider2D>();
        if (combatAnimation == null) combatAnimation = GetComponent<PlayerCombatAnimation>() ?? GetComponentInParent<PlayerCombatAnimation>();
        if (animationPresenter == null) animationPresenter = GetComponent<PlayerAnimationPresenter>() ?? GetComponentInParent<PlayerAnimationPresenter>();
        if (fallbackAnimator == null) fallbackAnimator = GetComponent<Animator>() ?? GetComponentInParent<Animator>();
